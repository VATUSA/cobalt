...
func CreateSoloCert(c *echo.Context) error {
	...
	if len(existing) > 0 {
		return RespondError(c, http.StatusConflict, errors.New("this controller already has an active solo cert for this position"))
	}
...
func UpdateSoloCert(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusNotFound, errors.New("solo cert not found"))
	}
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to update solo cert"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("solo cert not found"))
	}
	return RespondSuccess(c, int(certId))
}
...
func DeleteSoloCert(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to delete solo cert"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("solo cert not found"))
	}
