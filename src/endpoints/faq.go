...
func UpdateFaqCategory(c *echo.Context) error {
	...
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("faq category not found"))
	}
...
func DeleteFaqCategory(c *echo.Context) error {
	...
	if count > 0 {
		return RespondError(c, http.StatusConflict, errors.New("category still has items"))
	}
...
func CreateFaqItem(c *echo.Context) error {
	...
	if request.FaqCategoryId <= 0 {
		return RespondError(c, http.StatusBadRequest, errors.New("faq_category_id is required"))
	}
	...
	if _, err := dbconn.Queries().GetFaqCategoryById(ctx, int32(request.FaqCategoryId)); err != nil {
		return RespondError(c, http.StatusBadRequest, errors.New("faq_category_id does not exist"))
	}
...
func UpdateFaqItem(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to update item"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("faq item not found"))
	}
	return RespondSuccess(c, int(itemId))
}
...
func DeleteFaqItem(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to delete item"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("faq item not found"))
	}
