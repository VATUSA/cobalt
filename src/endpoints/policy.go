...
func UpdatePolicyCategory(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to update category"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("policy category not found"))
	}
	return RespondSuccess(c, int(categoryId))
}
...
func DeletePolicyCategory(c *echo.Context) error {
	...
	if count > 0 {
		return RespondError(c, http.StatusConflict, errors.New("category still has documents"))
	}

	result, err := dbconn.Queries().DeletePolicyCategory(ctx, categoryId)
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to delete category"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("policy category not found"))
	}
...
func CreatePolicyDocument(c *echo.Context) error {
	...
	if _, err := dbconn.Queries().GetPolicyCategoryById(ctx, int32(request.PolicyCategoryId)); err != nil {
		return RespondError(c, http.StatusBadRequest, errors.New("policy_category_id does not exist"))
	}
	...
	if err != nil {
		return RespondError(c, documentURLStatus(err), err)
	}
...
func UpdatePolicyDocument(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusNotFound, errors.New("policy document not found"))
	}
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to update policy document"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("policy document not found"))
	}
	return RespondSuccess(c, int(documentId))
}
...
func DeletePolicyDocument(c *echo.Context) error {
	...
	if err != nil {
		return RespondError(c, http.StatusInternalServerError, errors.New("failed to delete policy document"))
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
		return RespondError(c, http.StatusNotFound, errors.New("policy document not found"))
	}
